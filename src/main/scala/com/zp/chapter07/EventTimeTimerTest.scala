    ...
    stream.keyBy( data => "true")
      .process(new KeyedProcessFunction[String,Event,String] {
        override def processElement(i: Event, context: KeyedProcessFunction[String, Event, String]#Context, collector: Collector[String]): Unit = {
          val currenTime = context.timerService().currentWatermark()
          collector.collect(s"数据到达时间是：$currenTime,当前数据时间是：${i.timestamp}")
          //注册一个5秒的定时器
          context.timerService().registerEventTimeTimer(currenTime+5*1000)
        }
        //定义定时器触发时的执行逻辑
